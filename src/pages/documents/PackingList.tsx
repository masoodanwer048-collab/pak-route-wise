...
export default function PackingList() {
    ...
    const handleExport = (format: 'pdf' | 'excel' = 'excel') => {
        if (documents.length > 0) {
            // Batch export logic could go here, but for now let's just export the first one or alert
            // Or better yet, export the *list* as a report.
            // Since the user asked for functional download, let's keep the CSV for batch, 
            // but maybe add a helper to download specific selected ones?
            // Actually, the request "functional also download in pdf/excel" likely applied to the individual items as well.

            // Retain CSV export for list view
            const csv = [
                ['Invoice No', 'Vessel', 'Voyage', 'POL', 'POD', 'Shipper', 'Consignee', 'Weight', 'Packages', 'Status', 'Date'].join(','),
                ...documents.map((d) =>
                    [d.documentNumber, d.vesselFlightTruck, d.voyageFlightNo, d.pol, d.pod, d.shipper, d.consignee, d.weight, d.packages, d.status, d.issueDate].join(',')
                ),
            ].join('\n');
            ...
            a.href = url;
            a.download = `packing-list-report-${new Date().toISOString().split('T')[0]}.csv`;
            a.click();
            toast.success('List exported to CSV');
        }
    };

    const downloadDocument = (doc: ShippingDocument, format: 'pdf' | 'excel') => {
        toast.info(`Generating ${format.toUpperCase()}...`);
        try {
            if (format === 'pdf') {
                import('@/utils/documentGenerator').then(mod => mod.generatePDF(doc));
            } else {
                import('@/utils/documentGenerator').then(mod => mod.generateExcel(doc));
            }
        } catch (e) {
    ...
    return (
        <MainLayout title="Packing List" subtitle="Manage commercial packing lists and invoices">
            <div className="space-y-6">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="relative">
                            ...
                            <Input
                                placeholder="Search Invoice No, shipper..."
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button variant="accent" onClick={() => setIsCreateOpen(true)}>
                            ...
                            <span className="hidden sm:inline">Create PL</span>
                ...
                <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="stat-card border border-border">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold">{documents.reduce((acc, doc) => acc + doc.packages, 0)}</p>
                ...
                {/* PL Cards - Responsive Grid */}
                {documents.length === 0 ? (
                    <div className="rounded-xl border border-border bg-card p-12 text-center">
                        <FileText className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                        <h3 className="font-semibold mb-2">No Packing Lists found</h3>
                            ...
                            >
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="pt-2 border-t border-border">
                                        <p className="text-xs text-muted-foreground mb-1">Shipper / Exporter</p>
                                        <p className="font-medium truncate">{doc.shipper}</p>
                                    </div>

                                    <div className="grid grid-cols-3 gap-2 pt-2">
                                        <div>
                                            <p className="text-xs text-muted-foreground">Packages</p>
                                            <p className="font-mono font-medium">{doc.packages}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-muted-foreground">G.W (Kg)</p>
                                            <p className="font-mono text-sm truncate">{doc.weight}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-muted-foreground">Meas (CBM)</p>
                                ...
                                <div className="flex gap-2 mt-4 pt-4 border-t border-border">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="bg-popover">
                                            <DropdownMenuItem onClick={() => downloadDocument(doc, 'pdf')}>
                                                <FileText className="h-4 w-4 mr-2" />
                                                Download PDF
                                            </DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => downloadDocument(doc, 'excel')}>
                                                <Download className="h-4 w-4 mr-2" />
                                                Download Excel
                                            </DropdownMenuItem>
