...
export default function Bilty() {
  ...
  const handleCopy = (docNumber: string) => {
    ...
    toast.success('Bilty number copied to clipboard');
  };

  const handleExport = () => {
    const csv = [
      ['Bilty Number', 'Truck', 'Driver', 'Origin', 'Destination', 'Consignor', 'Consignee', 'Weight', 'Status', 'Date'].join(','),
      ...documents.map((d) =>
        [d.documentNumber, d.vesselFlightTruck, d.voyageFlightNo, d.origin, d.destination, d.shipper, d.consignee, d.weight, d.status, d.issueDate].join(',')
      ),
    ].join('\n');
    ...
    a.href = url;
    a.download = `bilty-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    toast.success('Export completed');
  };

  return (
    <MainLayout title="Bilty / Road Receipt" subtitle="Manage road consignment notes and truck receipts">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative">
              ...
              <Input
                placeholder="Search bilty, truck, driver..."
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="released">Delivered</SelectItem>
          ...
          <div className="flex gap-3">
            ...
            <Button variant="accent" onClick={() => setIsCreateOpen(true)}>
              ...
              <span className="hidden sm:inline">Create Bilty</span>
          ...
          <div className="stat-card border border-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Total Bilties</p>
              </div>
            </div>
          </div>
          <div className="stat-card border border-border">
                  ...
                  <tr key={doc.id}>
                    ...
                    <td>
                      <div className="flex flex-col">
                        <span className="text-sm font-medium font-mono">{doc.vesselFlightTruck}</span>
                    ...
                    <td className="truncate max-w-[120px]">{doc.shipper}</td>
                    <td className="truncate max-w-[120px]">{doc.consignee}</td>
                    <td className="font-mono text-sm">{doc.weight}</td>
                    <td className="font-mono text-sm">{doc.packages}</td>
                    <td>
                      <Badge variant="outline" className={cn('capitalize', statusColors[doc.status])}>
                        {doc.status === 'released' ? 'Delivered' : doc.status}
                      </Badge>
                    </td>
                    <td>
            ...
            {documents.map((doc) => (
              <div key={doc.id} className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-mono">{doc.vesselFlightTruck}</span>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Bilty</AlertDialogTitle>
            <AlertDialogDescription>
